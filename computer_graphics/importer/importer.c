/**
	Reads objects with regular vertices from a X3D file, looks its data and store then in list structures.
...
void insertFace(FaceList *list, const int coordinate[]) {
	...
	newFace->pointIndex[3] = coordinate[3];
...
void printFaceList() {
	...
	while(face != NULL){
		printf("%d %2d %2d %2d\n", face->pointIndex[0], face->pointIndex[1], face->pointIndex[2], face->pointIndex[3]);
		face = face->next;
	}
}
...
int readFace(FILE *inFile, const char* buffer){
	...
	if(pFirstOcurrence != NULL){ // Pattern was found
		...
		*/
		...
		int facePoint[POINTS_PER_FACE], unusedPoint;
		facePoint[0] = atoi(token);
		fscanf(inFile, "%d %d %d %d", &facePoint[1], &facePoint[2], &facePoint[3], &unusedPoint); // read three more face coordinates
		insertFace(faceList, facePoint);

		// gets the others coordinate values
		while(fscanf(inFile, "%d %d %d %d %d", &facePoint[0], &facePoint[1], &facePoint[2], &facePoint[3], &unusedPoint) > 0){
			insertFace(faceList, facePoint);
		}
